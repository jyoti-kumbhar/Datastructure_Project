...
class TSPGUI:
    def __init__(self, root):
        ...
        tk.Label(root, text="Places (comma-separated):", font=('Arial', 20), bg='light pink').grid(row=3, column=1, sticky='w')
        self.forts_entry = tk.Entry(root, width=50, font=("Arial", 20))
        self.forts_entry.grid(row=3, column=2, columnspan=2)

        # Input for distance matrix
        tk.Label(root, text="Distance matrix (comma-separated values, \nrows separated by semicolons):", font=('Arial', 20), bg='light pink').grid(row=4, column=1, sticky='w')
        self.matrix_entry = tk.Entry(root, width=50, font=("Arial", 20))
        self.matrix_entry.grid(row=4, column=2, columnspan=2)

        # Solve button
        self.solve_button = tk.Button(root, text="Solve TSP", command=self.solve_tsp, font=('Arial', 20),bg="light pink")
        self.solve_button.grid(row=5, column=2)

        # Output Text for Results
        self.output_text = tk.Text(root, height=10, width=60, font=("Arial", 20))
    ...
    def visualize_graph(self, route, distance_matrix, forts, min_distance):
        ...
        nx.draw_networkx_nodes(G, pos, node_size=700, node_color='light pink', ax=ax)
        nx.draw_networkx_edges(G, pos, edgelist=G.edges(), width=2, edge_color='black', ax=ax)
